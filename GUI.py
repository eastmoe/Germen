...
def show_extra(*args):
    ...
    if value in ["模拟点击"]:
        # 复选框放置
        checkUseOldImagePlot.grid(row=8, column=0, sticky="W")
        checkUseOldClickPlot.grid(row=9, column=0, sticky="W")
        # 放置按钮
        RegetpicturePlot.grid(row=8, column=1, sticky="E")
        RegetclickPlot.grid(row=9, column=1, sticky="E")
        #显示统一控件
        ShowControlPanel()
        # 隐藏之前的部件
    ...
    if value in ["模拟按键"]:
        # 放置文本标签
        labelkey.grid(row=9,column=0, sticky="W")
        # 放置输入框
        entryKey.grid(row=9,column=1, sticky="E")
        ...
        checkUseOldImagePlot.grid(row=8, column=0, sticky="W")
        # 放置按钮
        RegetpicturePlot.grid(row=8, column=1, sticky="W")
        # 显示统一控件
        ShowControlPanel()
        # 隐藏之前的部件
...
def ShowControlPanel():
    ...
    #sep.pack(fill=x)
    # 放置按钮
    StartCapture.grid(row=11, column=0, sticky="E")
    StopCapture.grid(row=11, column=1, sticky="E")
    Merge.grid(row=12, column=0, sticky="E")
    Format.grid(row=12, column=1, sticky="E")
    SimpleProcess.grid(row=13,columnspan = 2, sticky = tk.W+tk.E)
...
# 将输入框放置在第二列
entryPictureDir.grid(row=1, column=1, sticky="E")
entryOCROutPaDir.grid(row=2, column=1, sticky="E")
entryMergeBookDir.grid(row=3, column=1, sticky="E")
entryFinalNovelDir.grid(row=4, column=1, sticky="E")
entryCycle.grid(row=5, column=1, sticky="E")
entryTime.grid(row=6, column=1, sticky="E")
...
# 放置加载按钮
loadConfigjson.grid(row=0, column=1, sticky="W")
