...
- `OpenAIOCR.py`：统一 OCR 后端，可走通用 OpenAI 兼容 VLM，也可走专用图片-only OCR 模型。
...
6. `OCRBackend`：`通用VLM路径` 或 `专用OCR路径`。
7. `OpenAIURL`：OpenAI 兼容 API Base URL，官方接口通常为 `https://api.openai.com/v1`；本地 Unlimited-OCR 常用 `http://127.0.0.1:8080/v1`。
8. `OpenAIAPIKEY`：OpenAI 兼容 API Key。第三方付费服务可在这里填写；本地服务不需要密钥时可以留空。
9. `OpenAIOCRModel`：OCR 模型。通用路径默认 `gpt-4.1-mini`；选择专用 OCR 路径时界面会自动填入 `Unlimited-OCR`，仍可手动改成服务实际暴露的模型名。
10. `OpenAIOCRPrompt`：通用 VLM OCR 提示词；选择专用 OCR 路径时界面和后台会自动清空，真实请求不会发送提示词。
11. `DedicatedOCRStripCoordinates`：专用 OCR 返回 `text [x, y, x, y]正文` 时是否在后台剥离坐标前缀，默认 `true`。
12. `CaptureSource`：`屏幕区域` 或 `图像输入源`。
13. `InputSource`：图像输入源编号，通常从 `0` 开始。
14. `ADBSerial`：ADB 设备序列号。单设备连接时可以留空，由程序自动识别。
15. `ADBTapX` / `ADBTapY`：ADB 模拟点击学习到的屏幕坐标。
16. `PageMethod`：`模拟按键`、`模拟点击`、`音量下` 或 `模拟点击学习`。
17. `PageKey`：模拟按键翻页时发送的按键。
18. `CapturePages`：计划采集页数。
19. `WebUIPassword`：WebUI 访问密码，默认值为 `germen`，请在首次使用前修改。
...
4. 在 GUI 中选择 OCR 路径，配置 OpenAI 兼容 API、模型、截图目录和输出目录；两条路径共用同一组 URL、Key 和模型字段。
5. 选择采集来源：
